...
//#define INPUT_SHAPING_Z
  ...
  // Prevent enabling LCD input-shaping menus when firmware-level input shaping is enabled.
  // Enabling both can cause conflicts or double configuration; keep LCD menus disabled when
  // INPUT_SHAPING_[XYZ] are enabled. If you really need the menus, disable INPUT_SHAPING_* first.
  #if ANY(INPUT_SHAPING_X, INPUT_SHAPING_Y, INPUT_SHAPING_Z)
    #if ENABLED(SHAPING_MENU) || ENABLED(DWIN_INPUT_SHAPING_MENU)
      #error "Do not enable LCD input shaping menus (SHAPING_MENU or DWIN_INPUT_SHAPING_MENU) when INPUT_SHAPING_* is enabled in firmware."
    #endif
      ...
      #define SHAPING_FREQ_Y  36.0        // (Hz) The default dominant resonant frequency on the Y axis.
      #define SHAPING_ZETA_Y   0.05       // Damping ratio of the Y axis (range: 0.0 = no damping to 1.0 = critical damping).
    #endif
